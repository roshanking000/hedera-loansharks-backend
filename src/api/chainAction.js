...
const PAL_TOKEN_ID = '0.0.1690594'
const palDecimals = 8;
...
exports.receiveAllowanceNft = async (sender, tokenId, serialNum) => {
  console.log('receiveAllowanceNft log - 1 : ', sender, tokenId, serialNum);
  try {
    const _nft = new NftId(TokenId.fromString(tokenId), serialNum);

    const nftSendTx = new TransferTransaction().addApprovedNftTransfer(_nft, AccountId.fromString(sender), operatorId);
...
exports.repay = async (lender_, borrower_, loanAmount_, repayAmount_, tokenId_, serialNumber_) => {
  try {
    const _repayHbarAmount = new Hbar(repayAmount_);
    let _treasuryAmount = parseFloat(((repayAmount_ - loanAmount_) * 2) / 5).toFixed(3);
    console.log(_treasuryAmount);
    const _treasuryLoanInterestAmount = new Hbar(_treasuryAmount);
    let _lenderAmount = parseFloat(repayAmount_ - _treasuryAmount).toFixed(3);
    console.log(_lenderAmount);
    const _lenderLoanInterestAmount = new Hbar(_lenderAmount);
    console.log(_repayHbarAmount, _lenderLoanInterestAmount, _treasuryLoanInterestAmount);

    const transferTx = new TransferTransaction()
      .addApprovedHbarTransfer(AccountId.fromString(borrower_), _repayHbarAmount.negated())
      .addHbarTransfer(operatorId, _treasuryLoanInterestAmount)
      .addHbarTransfer(AccountId.fromString(lender_), _lenderLoanInterestAmount)
      .addNftTransfer(TokenId.fromString(tokenId_), serialNumber_, operatorId, AccountId.fromString(borrower_))
      .setTransactionId(TransactionId.generate(operatorId))
      .freezeWith(client);

    const approvedSendSign = await transferTx.sign(operatorKey);
    const transferSubmit = await approvedSendSign.execute(client);
    const transferRx = await transferSubmit.getReceipt(client);
...
exports.allowanceNft = async (_accountId, _tokenId, _serialNumber) => {
  try {
    ...
    const transaction = new AccountAllowanceApproveTransaction()
      .approveTokenNftAllowance(_nft, operatorId, AccountId.fromString(_accountId))
      .freezeWith(client);
